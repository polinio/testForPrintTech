...
function handleQuestions() { 
    // Создаём массив с вопросами в случайном порядке
    shuffledQuestions = [];
...
let tpScore = 0; // Счёт по термографической печати
let stScore = 0; // Счёт по струйной печати
let teScore = 0; // Счёт по терминологии
let wrongAttempt = 0; // Количество ошибок
let indexNumber = 0; // Индекс текущего вопроса

// Отображает следующий вопрос и номер текущего вопроса
...
function checkForAnswer() {
    ...
    const currentQuestionAnswer = currentQuestion.correctOption; // Правильный ответ
    const options = document.getElementsByName("option"); // Все радио-кнопки
    let correctOption = null;

    options.forEach((option) => {
        if (option.value === currentQuestionAnswer) {
            // Находим радио-кнопку с правильным ответом
    ...
    options.forEach((option) => {
        if (option.checked === true && option.value === currentQuestionAnswer) {
            ...
            playerScore++; // Увеличиваем общий счёт
            indexNumber++; // Переходим к следующему вопросу
...
// Отображает статистику по завершению теста
function handleEndGameStats() {
    const playerGrade = ((playerScore / questions.length) * 100).toFixed(2);
...
// Отображает ранги по категориям
function handleEndGameRank() {
    let remark_el = null, remarkColor_el = null, rank_el = null;
    let remark_tp = null, remarkColor_tp = null, rank_tp = null;
